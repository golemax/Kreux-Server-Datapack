execute as @s[tag=!authorize] run tellraw @s Vous n'avez pas la permission de réinitialiser le jeu.

execute store result score @s tempo run data get storage gamemode:config authorizeplayernum

execute if score @s tempo matches ..1 as @s[tag=authorize] run tellraw @s Le nombre de joueurs pouvant vérifier votre requête est insuffisant.
