...
execute as @s[tag=!RESET,tag=!NORESET,tag=authorize,tag=!NOTWOPLAYER] run function gamemode:reset
...
execute as @s[tag=RESET,tag=authorize,tag=!OTHERSNORESET,tag=!OTHERSRESET,tag=!NOTWOPLAYER] run function gamemode:reset
...
#If require more players
tellraw @s[tag=NOTWOPLAYER,tag=authorize] {"text":"Not enough players allowed","color":"red","bold":false}
execute as @s[tag=NOTWOPLAYER,tag=authorize] run scoreboard players set @s skipline 1
...
execute at @s[tag=RESET,tag=OTHERSRESET,tag=authorize] run tellraw @a[tag=authorize,tag=!RESET] {"text":" \nThe request has accepted","color":"red"}
